...
# Anchor every path to the justfile's directory so recipes work from any CWD.
ROOT := justfile_directory()
bin_name := "pp"
# Static build target; override on the command line, e.g.:
#   just TARGET=aarch64-unknown-linux-musl build-static
...
build-static:
    cargo build --release --target {{TARGET}} -p pp-cli

# Build the FFI cdylib for THIS machine (-C target-cpu=native).
# For a portable build (any CPU) use `just nvim-portable`.
build-nvim-native:
    RUSTFLAGS="-C target-cpu=native" cargo build -p pp-nvim --release
    @mkdir -p {{ROOT}}/build
    cp {{ROOT}}/target/release/libpp_nvim.so {{ROOT}}/build/libpp_nvim.so
    @echo "✓ libpp_nvim.so (native) → {{ROOT}}/build/libpp_nvim.so"

# Portable FFI cdylib — runs on any CPU (no -C target-cpu=native).
nvim-portable:
    cargo build -p pp-nvim --release
    @mkdir -p {{ROOT}}/build
    cp {{ROOT}}/target/release/libpp_nvim.so {{ROOT}}/build/libpp_nvim.so
    @echo "✓ libpp_nvim.so (portable) → {{ROOT}}/build/libpp_nvim.so"
...
install-static: build-static
    @mkdir -p {{install_dir}}
    cp -f {{ROOT}}/target/{{TARGET}}/release/{{bin_name}} {{install_dir}}/{{bin_name}}
...
install-bin:
    @mkdir -p {{install_dir}}
    cp {{ROOT}}/target/release/{{bin_name}} {{install_dir}}/{{bin_name}}
    @echo "✓ Binary → {{install_dir}}/{{bin_name}}"

install-fish:
    @mkdir -p {{fish_func_dir}}
    cp {{ROOT}}/scripts/pp.fish {{fish_func_dir}}/pp.fish
...
install-fish-completions:
    ...
    {{ROOT}}/target/release/{{bin_name}} generate fish > {{fish_comp_dir}}/pp.fish
    @echo "✓ Fish completions → {{fish_comp_dir}}/pp.fish"

install-bash-completions:
    @mkdir -p {{bash_comp_dir}}
    {{ROOT}}/target/release/{{bin_name}} generate bash > {{bash_comp_dir}}/pp
    @echo "✓ Bash completions → {{bash_comp_dir}}/pp"

install-zsh-completions:
    @mkdir -p {{zsh_comp_dir}}
    {{ROOT}}/target/release/{{bin_name}} generate zsh > {{zsh_comp_dir}}/_pp
...
index:
    {{ROOT}}/target/release/{{bin_name}} index

# Clear the repository index
clear:
    {{ROOT}}/target/release/{{bin_name}} clear
...
list:
    {{ROOT}}/target/release/{{bin_name}} list
